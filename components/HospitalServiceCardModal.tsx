...
import type { HospitalServiceAppointmentCard } from '../types';
import { CloseIcon, QrCodeIcon, HospitalIcon, InformationCircleIcon } from './IconComponents';

interface HospitalServiceCardModalProps {
  card: HospitalServiceAppointmentCard;
  onClose: () => void;
}

const HospitalServiceCardModal: React.FC<HospitalServiceCardModalProps> = ({ card, onClose }) => {
  return (
        ...
        <div className="p-8">
            <div className="bg-gradient-to-br from-teal-500 to-cyan-500 p-6 rounded-xl shadow-lg text-white">
                <div className="flex justify-between items-start border-b border-white/20 pb-4">
                    <div>
                        <p className="text-xs uppercase opacity-80 font-semibold tracking-wider">Hospital Service</p>
                        <h2 className="text-2xl font-bold">{card.serviceName}</h2>
                    </div>
                     <HospitalIcon className="h-8 w-8 text-white/70" />
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                    <div>
                     ...
                     <div>
                        <p className="text-xs uppercase opacity-80 font-semibold tracking-wider">Hospital</p>
                        <p className="font-medium">{card.hospitalName}</p>
                    </div>
                    <div>
...
export default HospitalServiceCardModal;
